// ============================================================================
// petlo - WeightInputField
// ============================================================================
//
// 体重入力フィールド。kg / lb の単位切替トグル付き。
//
// 動作:
//   - 数値テキスト入力 (小数2桁許可)
//   - 単位切替で内部値(g)を保持したまま表示単位を切替
//   - 保存時は g 単位 int を返す
//
// 親が State 管理:
//   - weightG: 内部保存値
//   - unit: 表示単位(永続化はユーザー設定で別途管理)
//
// ============================================================================

import 'package:flutter/material.dart';
import 'package:flutter/services.dart';

import '../../../core/theme/app_colors.dart';
import '../../../core/theme/app_dimensions.dart';
import '../../../core/theme/app_typography.dart';
import '../../../core/utils/unit_converters.dart';
import '../../../core/widgets/eyebrow_text.dart';
import '../../../data/local/database_enums.dart';

class WeightInputField extends StatefulWidget {
  const WeightInputField({
    required this.weightG,
    required this.unit,
    required this.onWeightChanged,
    required this.onUnitChanged,
    this.label = 'Weight',
    this.required = false,
    this.errorText,
    super.key,
  });

  final int? weightG;
  final WeightUnit unit;
  final ValueChanged<int?> onWeightChanged;
  final ValueChanged<WeightUnit> onUnitChanged;
  final String label;
  final bool required;
  final String? errorText;

  @override
  State<WeightInputField> createState() => _WeightInputFieldState();
}

class _WeightInputFieldState extends State<WeightInputField> {
  late final TextEditingController _controller;
  WeightUnit? _lastUnit;

  @override
  void initState() {
    super.initState();
    _controller = TextEditingController(text: _displayText());
    _lastUnit = widget.unit;
  }

  @override
  void didUpdateWidget(WeightInputField old) {
    super.didUpdateWidget(old);
    // 単位切替時は表示テキストを更新
    if (old.unit != widget.unit) {
      _controller.text = _displayText();
      _lastUnit = widget.unit;
    } else if (old.weightG != widget.weightG && _controller.text.isEmpty) {
      // 外部から値が初期化された場合(編集モードのロード後)
      _controller.text = _displayText();
    }
  }

  @override
  void dispose() {
    _controller.dispose();
    super.dispose();
  }

  String _displayText() {
    if (widget.weightG == null) return '';
    return WeightConverter.formatG(
      weightG: widget.weightG!,
      unit: widget.unit,
    );
  }

  @override
  Widget build(BuildContext context) {
    final AppColors colors = AppColors.of(context);
    final AppTypography typo = AppTypography.of(context);
    final bool hasError = widget.errorText != null && widget.errorText!.isNotEmpty;

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        Row(
          children: <Widget>[
            EyebrowText(widget.label),
            if (widget.required) ...<Widget>[
              const SizedBox(width: 4),
              Text('*',
                  style: TextStyle(
                    color: colors.accentDanger,
                    fontFamily: 'JetBrainsMono',
                    fontSize: 10,
                  )),
            ],
            const Spacer(),
            // 単位切替トグル (kg / lb)
            _UnitToggle(
              currentUnit: widget.unit,
              onChanged: widget.onUnitChanged,
            ),
          ],
        ),
        const SizedBox(height: AppDimensions.gapSmall),

        // 大きな数値入力 (フラットなスタイル)
        TextField(
          controller: _controller,
          keyboardType: const TextInputType.numberWithOptions(decimal: true),
          inputFormatters: <TextInputFormatter>[
            // 数字 + ドット/カンマ + 小数2桁制限
            FilteringTextInputFormatter.allow(RegExp(r'[0-9.,]')),
          ],
          style: TextStyle(
            fontFamily: 'Fraunces',
            fontStyle: FontStyle.italic,
            fontSize: 32,
            height: 1.0,
            color: colors.fg,
            fontFeatures: <FontFeature>[
              const FontFeature.tabularFigures(),
            ],
          ),
          decoration: InputDecoration(
            hintText: '0',
            hintStyle: TextStyle(
              fontFamily: 'Fraunces',
              fontStyle: FontStyle.italic,
              fontSize: 32,
              color: colors.fgFaint,
            ),
            suffixText: WeightConverter.label(widget.unit),
            suffixStyle: typo.bodyMedium.copyWith(color: colors.fgMuted),
            border: _underline(hasError ? colors.accentDanger : colors.line),
            enabledBorder:
                _underline(hasError ? colors.accentDanger : colors.line),
            focusedBorder: _underline(
              hasError ? colors.accentDanger : colors.fg,
              width: 2,
            ),
            contentPadding:
                const EdgeInsets.symmetric(vertical: 8, horizontal: 0),
            isDense: true,
          ),
          onChanged: (String v) {
            final int? g = WeightConverter.parseToG(input: v, unit: widget.unit);
            widget.onWeightChanged(g);
          },
        ),

        if (hasError) ...<Widget>[
          const SizedBox(height: 4),
          Text(
            widget.errorText!,
            style: typo.bodySmall.copyWith(color: colors.accentDanger),
          ),
        ],
      ],
    );
  }

  UnderlineInputBorder _underline(Color color, {double width = 1}) {
    return UnderlineInputBorder(
      borderSide: BorderSide(color: color, width: width),
    );
  }
}

class _UnitToggle extends StatelessWidget {
  const _UnitToggle({
    required this.currentUnit,
    required this.onChanged,
  });

  final WeightUnit currentUnit;
  final ValueChanged<WeightUnit> onChanged;

  @override
  Widget build(BuildContext context) {
    final AppColors colors = AppColors.of(context);
    return Row(
      children: <Widget>[
        for (final WeightUnit u in WeightUnit.values)
          InkWell(
            onTap: () => onChanged(u),
            child: Container(
              padding:
                  const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
              decoration: BoxDecoration(
                color: currentUnit == u ? colors.fg : colors.bg,
                border: Border.all(color: colors.fg, width: 1),
              ),
              child: Text(
                WeightConverter.label(u),
                style: TextStyle(
                  fontFamily: 'JetBrainsMono',
                  fontSize: 9,
                  letterSpacing: 9 * 0.15,
                  fontWeight: FontWeight.w500,
                  color: currentUnit == u ? colors.bg : colors.fg,
                ),
              ),
            ),
          ),
      ],
    );
  }
}
